...
func mealNumber(mealName:String)->Meal?{
  return meals[mealName]
    
}
mealNumber(mealName: "Egg Whites")
mealNumber(mealName: "Orange Juice")

if let mealName = meals["Breakfast"]{
...
func check()->[String:Any]{
    guard let mealLogIn =  UserDefaults.standard.dictionary(forKey: "mealLog") else{
    return[:]
